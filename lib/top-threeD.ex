...
##Initiate Gossip or pushsum based on SecondArgument
  def init([x,y,z,n, gossipOrpushSum]) do
    ...
    case gossipOrpushSum do
      ...
      1 -> {:ok, [Active,0, 0, 0, 0, x, 1, n*n , x, y, z| neighbors] } #Denotes [STatus of an Actor, received count,pushsumStreak,prevousSW,to_terminate, s, w, n, self_number_id-x,y,z | neighbors ]
    end
  end
...
#Creating Network ThreeDTopology
  ...
  # Receiving
  def handle_cast({:message_push_sum, {receive_s, receive_w} }, [status,count,pushsumStreak,prevousSW,term, s ,w, n, x, y,z | neighbors ] = state ) do
    length = round(Float.ceil(:math.sqrt(n)))
    GenServer.cast(Master,{:received, [{x,y,z}]})
      case abs(((s+receive_s)/(w+receive_w))-prevousSW) < :math.pow(10,-10) do
        false ->push_sum((s+receive_s)/2,(w+receive_w)/2,neighbors,self(),x,y,z)
                {:noreply,[status,count+1, 0, (s+receive_s)/(w+receive_w), term, (s+receive_s)/2, (w+receive_w)/2, n, x, y,z  | neighbors]}
        true ->
          case pushsumStreak + 1 == 3 do
            true ->  GenServer.cast(Master,{:hibernated, [{x,y,z}]})
                      {:noreply,[status,count+1, pushsumStreak+1, (s+receive_s)/(w+receive_w), 1, (s+receive_s), (w+receive_w), n, x, y,z  | neighbors]}
            false -> push_sum((s+receive_s)/2,(w+receive_w)/2,neighbors,self(),x,y,z)
                      {:noreply,[status,count+1, pushsumStreak+1, (s+receive_s)/(w+receive_w), 0, (s+receive_s)/2, (w+receive_w)/2, n, x, y,z  | neighbors]}
          end
        end
  end
  ...
  # Handling failure scenario - Retry sending
  def handle_cast({:retry_push_sum, {receive_s, receive_w,actorId} }, [status,count,pushsumStreak,prevousSW,term, s ,w, n, x, y,z | neighbors ] = state ) do
    push_sum(receive_s,receive_w,neighbors,actorId ,x,y,z)
    {:noreply,state}
  end
  ...
  def handle_call(:is_active,_from, state) do
    {status,x,y,z} =
      case state do
        [status,count,pushsumStreak,prevousSW,0, s ,w, n, x, y,z | neighbors ] -> {status,x,y,z}
